...
function Cursor({ ref, children, className, style, ...props }: CursorProps) {
  ...
  const [isOverClickable, setIsOverClickable] = React.useState(false);
  
  // Cursore diretto senza stabilizzatore - come Apple
  const springX = x;
  const springY = y;
    ...
    if (parentElement && isActive) {
      // Escludi il dock dal cursore personalizzato
      const dockElements = parentElement.querySelectorAll('[role="toolbar"]');
      ...
      {isActive && (
        <motion.div
          ...
          className={cn(
            ...
            isOverClickable && 'drop-shadow-[0_0_10px_rgba(0,122,255,0.8)]',
            className,
          )}
          style={{ 
            top: springY, 
            left: springX, 
            filter: isOverClickable ? 'drop-shadow(0 0 8px rgba(0,122,255,0.6))' : 'none',
            ...style 
          }}
          initial={{ scale: 0, opacity: 0 }}
          animate={{ 
            scale: isOverClickable ? 1.1 : 1, 
            opacity: 1 
          }}
